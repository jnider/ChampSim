...
static unsigned long artificial;
static unsigned long counter;
static unsigned long	total_accesses;
static unsigned int accesses[LLC_WAYS];
...
char rlog_name[] = "output_hawkj.csv";
FILE *rlog;
...
void CACHE::llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
					fprintf(rlog, "%u,0x%lx,%u,0x%x,0x%lx,0x%lx,0x%lx,%u,%u\n",
						cpu, 0UL, set, way,
						0UL, paddr, PC, type, hit);

    paddr = (paddr >> 6) << 6;

	total_accesses++;
	accesses[way]++;
	if (total_accesses % 512 == 0)
	{
		for (int i=0; i < LLC_WAYS; i++)
			accesses[i] /= LLC_WAYS;
		printf("Accesses: [%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u][%u]\n",
			accesses[0], accesses[1], accesses[2], accesses[3], accesses[4], accesses[5], accesses[6], accesses[7],
			accesses[8], accesses[9], accesses[10], accesses[11], accesses[12], accesses[13], accesses[14], accesses[15]);
	}
    ...
    if(!prediction){
        rrip[set][way] = 0;// MAXRRIP;
        printf("No prediction!!\n");
    }
    else{

       // if (total_accesses % 128 == 0)
        //    rrip[set][way] = rand() % 3;
        //else
            rrip[set][way] = rand() % (MAXRRIP-1);
...
void CACHE::llc_replacement_final_stats()
{
    ...
    cout<< "Artificial replacements: " << artificial << endl;
